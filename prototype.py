...
if __name__ == "__main__":
    ...
    if typexml == '1':
        namexml = 'igra.xml'
        path = os.path.abspath(os.curdir) + os.path.sep + 'Игра' + os.path.sep
        tree = ET.parse(path+namexml)
        doc = Prototype(tree)
    elif typexml == '2':
        namexml = 'club.xml'
        path = os.path.abspath(os.curdir) + os.path.sep + 'Клуб' + os.path.sep
        tree = ET.parse(path + namexml)
        doc = Prototype(tree)
    elif typexml == '3':
        namexml = 'commander.xml'
        path = os.path.abspath(os.curdir) + os.path.sep + 'Командир' + os.path.sep
        tree = ET.parse(path + namexml)
        doc = Prototype(tree)
